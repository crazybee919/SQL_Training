...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query =` SELECT
                         CAST(ROUND(SUM(budget_adjusted), 2) AS DECIMAL(12, 3)) AS total_budget,
                         CAST(ROUND(SUM(revenue_adjusted), 2) AS DECIMAL(12, 3)) AS total_revenue
        FROM
                          ${MOVIES}
        WHERE
                          budget IS NOT NULL
  ...
  it(
    ...
    async done => {
      const query = ` SELECT COUNT(*) AS count FROM ${MOVIES} WHERE homepage IS NOT NULL AND (homepage) LIKE 'https%' `;
      const result = await db.selectSingleRow(query);
  ...
  strftime('%m', time_created) AS month FROM ${MOVIE_RATINGS} GROUP BY month ORDER BY count DESC `;
      const result = await db.selectMultipleRows(query);
