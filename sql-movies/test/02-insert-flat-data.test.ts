...
const insertMovies = (movies: Movie[]) => {
    return (
        `INSERT INTO MOVIES (imdb_id, popularity, budget, revenue, original_title, homepage, tagline, overview, runtime, release_date, budget_adjusted, revenue_adjusted) VALUES` +
        movies.map(movie => `('${escape(movie.imdbId)}', ${movie.popularity}, ${movie.budget}, ${movie.revenue}, '${escape(movie.originalTitle)}', '${escape(movie.homepage)}', ${movie.tagline ? `'${escape(movie.tagline)}'` : 'NULL'}, '${escape(movie.overview)}', ${movie.runtime}, '${escape(movie.releaseDate)}', ${movie.budgetAdjusted}, ${movie.revenueAdjusted})`).join(",")
    );
};
