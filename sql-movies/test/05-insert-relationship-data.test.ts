...
import {
    ...
    MOVIE_DIRECTORS,
    MOVIE_KEYWORDS,
    MOVIE_PRODUCTION_COMPANIES, GENRES, PRODUCTION_COMPANIES
...
const insertMovieGenres = (
    ...
    const actualGenreRows = genreRows.filter(row => genres.includes(row.genre));
    return `INSERT INTO MOVIE_GENRES (movie_id, genre_id) VALUES `
        + actualGenreRows.map(row => `(${movieId}, ${row.id})`).join(",");
};
const insertMovieActors = (
    ...
    const actualActorRows = actorRows.filter(row => actors.includes(row.full_name));
    return `INSERT INTO MOVIE_ACTORS (movie_id, actor_id) VALUES`
        + actualActorRows.map(row => `(${movieId},${row.id})`).join(", ");
};
...
const insertMovieDirectors = (
  ...
  directorRows: DirectorRow[]
    ...
    const actualDirectorRows = directorRows.filter(row => directors.includes(row.full_name));
    return `INSERT INTO MOVIE_DIRECTORS (movie_id, director_id) VALUES`
        + actualDirectorRows.map(row => `(${movieId},${row.id})`).join(", ");
};
...
const insertMovieKeywords = (
  ...
  keywordRows: KeywordRow[]
    ...
    const actualKeywordRows = keywordRows.filter(row => keywords.includes(row.keyword));
    return `INSERT INTO MOVIE_KEYWORDS (movie_id, keyword_id) VALUES`
        + actualKeywordRows.map(row => `(${movieId},${row.id})`).join(", ");
};
...
const insertMovieProductionCompanies = (
  ...
  productionCompanyRows: ProductionCompanyRow[]
    ...
    const actualProductionCompanyRows = productionCompanyRows.filter(row => productionCompanies.includes(row.company_name));
    return `INSERT INTO MOVIE_PRODUCTION_COMPANIES (movie_id, company_id) VALUES`
        + actualProductionCompanyRows.map(row => `(${movieId},${row.id})`).join(", ");
};
